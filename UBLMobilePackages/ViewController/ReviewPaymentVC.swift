...
class ReviewPaymentVC: HeaderTitleViewController {
    ...
    private let dateValue = GenericLabel(myfont: .systemFont(ofSize: 17, weight: .semibold), color: .white, backgroundClr: .clear, mytext: "30/06/2003", myNumberOfLines: 1, clickable: false, txtAlignment: .left, cornerRadius: 0)
    ...
    private let payNowBtn = GenericButton(bgColor: .systemBlue, tintClr: .white, title: "Pay Now", imageSize: 0, labelFont: .systemFont(ofSize: 16, weight: .semibold), cornrRadius: 12)
    ...
    private func configureUI() {
        ...
        self.dateValue.text = formatter.string(from: date)
    ...
    @objc private func payNowClicked() {
        let v = ConfirmPaymentV(amount: packageModel?.amount ?? "", name: payeesModel?.payeeName ?? "", number: payeesModel?.number ?? "")
        view.addSubview(v)
